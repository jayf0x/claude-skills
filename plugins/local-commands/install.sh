...
mkdir -p "${COMMANDS_DIR}" "${CACHE_DIR}/scripts"
...
cp "${SCRIPT_DIR}/commands/cmds-harvest.md" "${COMMANDS_DIR}/cmds-harvest.md"
cp "${SCRIPT_DIR}/commands/cmds-tune.md" "${COMMANDS_DIR}/cmds-tune.md"
cp "${SCRIPT_DIR}/scripts/extract-commands.py" "${CACHE_DIR}/scripts/extract-commands.py"
cp "${SCRIPT_DIR}/scripts/harvest_diff.py" "${CACHE_DIR}/scripts/harvest_diff.py"
cp "${SCRIPT_DIR}/scripts/harvest.test.py" "${CACHE_DIR}/scripts/harvest.test.py"
chmod +x "${CACHE_DIR}/scripts/extract-commands.py" "${CACHE_DIR}/scripts/harvest_diff.py" "${CACHE_DIR}/scripts/harvest.test.py"
...
echo "       /cmds-harvest   — one-time backfill: mine all past session transcripts"
echo "       /cmds-compress  — merge dumps into all-local-commands.md (auto-loaded each session)"
echo "       /cmds-tune      — audit whether the harvest filters are well-tuned"
echo "       ${CACHE_DIR}/scripts/harvest.test.py — standalone, unattended version of /cmds-tune (invokes 'claude' CLI, can cost API calls)"
echo ""
